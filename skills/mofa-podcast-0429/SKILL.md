...
Preset voices (built-in): douwt (default), yangmi

Custom/cloned voices: use the `podcast_voice_save` tool to register a reference audio (`.wav`) as a named voice, then reference it in scripts with `clone:` prefix (e.g., `clone:trump`).

Alternatively, you can manually place `.wav` files in the skill's `voice_profiles/` directory. The filename determines the voice name:
- `trump_ref.wav` → `clone:trump`
- `ellen.wav` → `clone:ellen`

**Adding a new cloned voice via conversation:**
1. Send a short reference audio (5–30s, WAV format, clean speech) to the agent
2. Say: "Save this as a cloned voice named 'xxx'"
3. The agent calls `podcast_voice_save` with the audio file path
4. After saving, the voice appears in `podcast_voices` and can be used in scripts
