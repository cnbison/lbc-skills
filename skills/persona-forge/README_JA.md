...
**Persona Forgeは誰の思考法でも蒸留できる。マスク、Naval、マンガー、ファインマンをあなたのために働かせよう。**
...
名前を入力するだけで、Persona Forgeが調査・抽出・検証の全プロセスを自動で行う。

[使用例](#使用例) · [インストール](#インストール) · [Persona Forgeが蒸留するもの](#nuwaが蒸留するもの) · [仕組み](#仕組み)
...
## Persona Forgeが蒸留するもの

各分野の最高の人物を蒸留するには、日常の作業習慣よりも深いものを抽出する必要がある。Persona Forgeは六層を抽出する:
...
作業習慣はプロセス文書で伝えられる。だが、マンガーとマスクが同じ問題に対して異なる結論を出す理由は、認知フレームワークにある。Persona Forgeが抽出するのは認知オペレーティングシステムだ。
...
名前を入力すると、Persona Forgeは四つのことを行う:
...
Persona Forgeは人をコピーしない。認知オペレーティングシステムを抽出する。

**Persona Forge（Persona Forge）**は、中国神話で泥から人間を創った女神だ。ここでの泥は公開情報であり、創られるのは人ではなく、鏡だ。

---

## 作者について

花叔（Huashu）/花生（Huasheng）、AI Native Coder、インディー開発者。すべての製品はAIが書いたコードで構築（[子猫フィルライト](https://apps.apple.com/app/id6738028637)が中国App Store有料チャート1位を達成）。Claude Codeで40+のカスタムスキルを実行中。Persona Forgeはスキルを作るスキルだ。
...
**Persona Forge** は人がどのように考えるかを蒸留する。<br><br>
*蒸留すべき次の人物は、同僚である必要はない。*
