...
**Persona Forge는 누구의 사고방식도 증류한다. 머스크, 나발, 멍거, 파인만을 당신을 위해 일하게 하라.**
...
이름 하나만 입력하면 Persona Forge가 조사, 추출, 검증 전 과정을 자동으로 처리한다.

[예시](#예시) · [설치](#설치) · [Persona Forge가 증류하는 것](#nuwa가-증류하는-것) · [작동 원리](#작동-원리)
...
## Persona Forge가 증류하는 것

각 분야 최고의 인물을 증류하려면 일상적인 작업 습관보다 더 깊은 것을 추출해야 한다. Persona Forge는 여섯 층을 추출한다:
...
작업 습관은 프로세스 문서로 전달할 수 있다. 하지만 멍거와 머스크가 같은 문제에 다른 결론을 내리는 이유는 인지 프레임워크에 있다. Persona Forge가 추출하는 것은 인지 운영체제다.
...
이름을 입력하면 Persona Forge는 네 가지를 한다:
...
Persona Forge는 사람을 복제하지 않는다. 인지 운영체제를 추출한다.

**여와(Persona Forge)**는 중국 신화에서 흙으로 인간을 창조한 여신이다. 여기서 흙은 공개 정보이고, 창조되는 것은 사람이 아니라 거울이다.

---

## 작가 소개

화叔(Huashu)/화생(Huasheng), AI Native Coder, 독립 개발자. 모든 제품은 AI가 작성한 코드로 구축([새끼고양이 보조등](https://apps.apple.com/app/id6738028637)이 중국 App Store 유료 차트 1위 달성). Claude Code에서 40개 이상의 커스텀 스킬 실행 중. Persona Forge는 스킬을 만드는 스킬이다.
...
**Persona Forge**는 사람이 어떻게 생각하는지를 증류한다.<br><br>
*증류할 다음 사람이 꼭 동료일 필요는 없다.*
