...
name: persona-forge
description: |
  Persona Forge人格化：输入人名/主题/甚至只是模糊需求，自动深度调研→思维框架提炼→生成可运行的人物Skill。
  两种入口：(1)明确人名→直接蒸馏 (2)模糊需求→诊断推荐→再蒸馏。
  触发词：「造skill」「蒸馏XX」「Persona Forge」「人格化」「XX的思维方式」「做个XX视角」「更新XX的skill」。
...
用户不知道该蒸馏谁，只有需求或困惑。这时Persona Forge的工作是**从需求反推最合适的蒸馏对象**。
...
Persona Forge：你说的决策主要是哪种场景？比如商业/投资决策，还是职业/人生方向的选择？

用户：主要是商业上的，比如要不要做某个产品、要不要接某个合作

Persona Forge：明白了，你的核心需求是「在信息不完整时快速做出高质量的商业判断」。
我推荐3个候选：
[展示候选推荐...]
```
...
| 创建者归属 | 固定内容：`> 本Skill由 [Persona Forge · 人格化引擎](https://github.com/alchaincyf/nuwa-skill) 生成` + `> 创建者：[花叔](https://x.com/AlchainHust)` |
...
1. Persona Forge无法从公开渠道搜到用户的思维框架，需要用户提供素材
